...
def largest_snailfish_magnitude(data):
    sfns = [json.loads(line) for line in data.split("\n")]
    magnitudes = [check_magnitude(add(*combi)) for combi in itertools.product(sfns, repeat=2)]
    return sorted(magnitudes, reverse=True)[0]
...
if __name__ == "__main__":
    ...
    with open("test_input.txt") as f:
        result = largest_snailfish_magnitude(f.read())
    assert result == 3993, f"result not as expected: {result}"

    print(largest_snailfish_magnitude(aocd.get_data(day=18, year=2021)))
