...
def build_path(edges, current_path, all_paths):
    ...
    if current_path[-1] != "end":
        for edge in edges:
            if edge[0] == current_path[-1] and (edge[1].isupper() or edge[1].islower() and occurrences[edge[1]] < 1):
...
if __name__ == "__main__":
    for t in ("test_input_1.txt", 10), ("test_input_2.txt", 19), ("test_input_3.txt", 226):
        with open(t[0]) as f:
            result = main(f)
